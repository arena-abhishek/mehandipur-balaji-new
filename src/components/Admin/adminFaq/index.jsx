    ...
    columnHelper.accessor("id", {
      ...
      cell: (info) => (
        <div className="flex gap-2">
          {/* <Link href={`/admin/adminFAQ/details?slug=${info.getValue()}`}>
            <button className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              <Eye className="w-4 h-4 text-gray-600 dark:text-gray-400" />
            </button>
          </Link> */}
          <Link href={`/admin/adminFaq/addEdit?id=${info.getValue()}`}>
            <button className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
    ...
    if (confirmDelete) {
      try {
        ...
        if (response.ok) {
          ...
          console.error("Error deleting FAQ:", errorData);
          alert("Failed to delete the FAQ. Please try again.");
        }
      } catch (error) {
      ...
      <div className="p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            ...
            <Link href="/admin/adminFaq/addEdit">
